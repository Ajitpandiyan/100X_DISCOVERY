...
def render_profile_list():
    ...
    if "profiles" not in result or not result["profiles"]:
        st.warning("No profiles found. Create some profiles using the Profile Creation tab!")
    ...
    # Add search/filter
    search_term = st.text_input("Filter profiles", placeholder="Type to filter by name, skills, or interests")
    if search_term:
        ...
        for p in profiles:
            ...
            if (search_term in name or 
                search_term in bio or
                any(search_term in skill for skill in skills) or
                any(search_term in interest for interest in interests)):
